...
Use **Data → Map folder** to choose a host subfolder beneath an operator-configured allowlist root, select read-only or read/write access, and map it to an idle managed agent in one operation. One mapping can become the agent's task working directory; `/workspace` remains its private durable volume. The control plane retains an internal source record for validation and compatibility, but the normal UI does not make operators manage that intermediate object. The folder picker returns only root labels and relative directories, while raw host roots and Docker volume names never reach the browser. Overlapping read/write leases are rejected, and storage mutation is administrator-only REST/UI functionality that is not registered as a control-plane MCP tool. Configure the narrowest practical roots with `ATTACHMENT_ROOTS_JSON`; see [`docs/data-attachments.md`](./docs/data-attachments.md).
...
flowchart LR
  ...
  C --> D["Scoped data-source registry"]
  D -->|"explicit RO/RW mount"| W1
  D -->|"explicit RO/RW mount"| W2
  D -->|"explicit RO/RW mount"| W3
```
...
- Administrators can register scoped host subfolders or managed volumes and attach them read-only or read/write at `/data/<name>`. Runtime replacement preserves the private login/workspace volumes, host paths stay deployment-side, and overlapping writers are rejected.
...
- Every agent carries durable, provider-neutral runtime limits — wall-clock timeout, idle timeout, termination grace, max harness turns, child-command timeout and ceiling, subagent concurrency and depth, and background-task policy — injected by the control plane with every task, interactive or scheduled, and not overridable per request.
- Agent Dock's own supervision and a harness's native limits are reported separately, control by control. The wall-clock bound, the idle bound, and graceful-then-forced process-tree termination apply to every adapter because the wrapper owns them. A turn cap or a subagent limit applies only where the harness exposes a control; Claude Code exposes all six and Codex and OpenCode expose none, and the agent page says which of the saved limits are not in force rather than letting the operator assume they are.
- A cancel, a wall-clock timeout, an idle timeout, and a worker shutdown all terminate the harness's whole process group — graceful signal first, `SIGKILL` once the grace period is spent — and the group is reaped after a task ends normally too, so a finished task cannot leave a descendant running. This is the failure the feature exists for: a child subtree idling at near-zero CPU while the parent looked healthy and the account kept being billed.
- A task reports whether it is running, stalled, terminating, or force-terminated, together with when it started, when it last produced output, and how long it has been silent; after the stream closes, the runtime still reports why the last task ended, so "cancelled", "hit its wall-clock limit", and "stalled past its idle limit" are distinguishable from a plain failure.
- Where a harness announces them, subagent and child-command starts and stops cross the wrapper as metadata-only events — an id, an agent-type or tool name, a status, a duration — never a delegated prompt, a command line, or a tool result.
...
| `GET`, `POST` | `/api/v1/agents/:id/attachments` | List mappings or atomically map an inline approved folder (reusable `dataSourceId` remains supported) |
| `PATCH`, `DELETE` | `/api/v1/agents/:id/attachments/:attachmentId` | Change access/purpose or detach a source, replacing the runtime safely |
...
The worker invokes `codex exec --dangerously-bypass-approvals-and-sandbox` by default because the Docker container is the POC's non-interactive execution boundary. The agent can modify its private workspace, run processes inside the worker, and use the worker's network. Attach a host source read/write only when live edits are intentional; never approve SSH keys, cloud credentials, browser profiles, the Docker socket, or other sensitive paths. Set `ALLOW_UNSANDBOXED=0` to use Codex's `workspace-write` sandbox instead, understanding that unattended tool execution may be more constrained.

The included Compose file retains one bootstrap worker for each adapter so schema-v1 installations can migrate without losing logins. Those bindings are labeled `shared-legacy`, and the control plane prevents newly created agents from attaching to an already-bound bootstrap runtime. All new managed agents provision an exclusive container and complete private volume set. One-time remote-worker pairing and tool installation remain distinct next capabilities. Scoped data attachments apply only to managed dedicated runtimes; arbitrary paths remain forbidden.
...
Runtime limits bound one task inside one authenticated runtime; they are not a budget. A turn cap, an idle bound, and a subagent ceiling reduce how much work a runaway task can do, but nothing here caps spend, and a fleet of agents on one subscription still multiplies usage by the number of agents. Subagent hierarchy is only observed as deeply as the harness announces it — the first level for Claude Code, not at all for Codex and OpenCode — so `maxSubagentDepth` is enforced by the harness but deeper nesting is not something Agent Dock can display. Fine-grained cancellation of an individual subagent or child command is not exposed, because no adapter offers a control for it; operators cancel the top-level task, which takes the whole process tree with it. Process-tree termination relies on POSIX process groups, so it is a Linux-container behaviour, and a descendant that deliberately leaves its process group is outside the boundary.
...
The experimental Claude usage source is the one place a provider credential is read by Agent Dock code rather than only by the vendor CLI. That read happens inside the worker, against the worker's own private auth volume; the token is never returned, logged, persisted, or sent to the control plane or browser, and only normalized quota windows cross the wrapper. The response is reduced to the quota fields before anything is written to the telemetry volume, so the account and billing state it also carries is not retained at rest. Agent Dock does not perform Claude OAuth token exchanges: an ordinary `claude -p` task remains the sole owner of the CLI's supported refresh behavior. When Claude Code rotates its worker-local credential after a quota authentication failure, the wrapper notices only file metadata changing and permits one immediate quota recovery read before restoring the normal polling floor. The source targets an endpoint Anthropic does not document and for which no third-party OAuth flow or scoped usage-only token exists, so it may break without notice and is disabled unless you set `CLAUDE_OAUTH_USAGE=1`. The account-profile endpoint, which returns names, email addresses, and organization identifiers, is deliberately not called.

Connector keys are managed alongside their connectors at `/connectors`: an API key is stored once and delivered only to agents whose definitions bind a placeholder to it. For an HTTP connector, its host list is checked before delivery; a local stdio process has no destination to check. Values are encrypted at rest with a per-credential data key wrapped by a key provider. **With the default provider the key comes from the control plane's own environment, so this protects a copied volume or a stray backup and nothing else** — anyone who can read this host can read the credentials. The envelope exists so a deployment with a hardware root of trust or an external key service can supply a provider instead without changing the stored format, and an operator-supplied key held only in memory is a supported shape at the cost of unattended agents being unable to apply after a restart. No key is generated automatically, because one written beside the data it protects would imply protection that does not exist. The delivery sideband is memory-only, but some vendor CLIs require resolved values in their own configuration inside the agent's private volume; the harness and Docker host administrator can read those files, and the wrapper still requires a fresh delivery after restart rather than trusting stale provider configuration.

For HTTP, a credential is released only for a host on its list, checked when configuration is applied rather than when it is written, so editing a connector's destination cannot redirect it — and widening the list itself requires re-supplying the value, since that is the same act as sending the credential somewhere new. An empty list is unrestricted, and a stdio binding skips the host check. Deleting a key requires typing its name back, because the value exists nowhere else.

**This allowlist covers stored keys used by HTTP connectors only.** A placeholder bound to the `MCP_SECRET_` namespace has no destination allowlist, and neither does a stored key handed to a local process. A definition carrying either can still run inside an agent container that has normal network access. Effort is better spent making a leaked credential cheap — narrow scope at the provider, a short host list where it applies — than on stronger encryption of a broad one.

Connector secrets for MCP servers are also provisioned under the `MCP_SECRET_` namespace and are the only variables an MCP definition can resolve, so a definition cannot name the runtime's own wrapper token, a provider home, or the Ollama endpoint. Three limits are worth stating plainly rather than leaving to inference. The namespace is forwarded to every managed runtime rather than only to agents that reference it, because provisioning precedes any MCP binding, so a connector secret is present in every agent container. The resolved value is readable by the agent there, so this makes a credential revocable in one place without being per-agent isolation or confidentiality against the harness. And there is no destination allowlist yet: a definition carrying a legitimately provisioned secret can still name any `https` host, so an operator authoring or approving a definition is the only thing deciding where that credential is sent.
...
Host data sources are a deliberate expansion of an agent's authority. Read-only is Docker-enforced; read/write means the harness can edit or delete everything under that exact source. Paths are constrained to configured roots, parent traversal and symlink components are rejected by a no-network helper running as the target worker UID, and overlapping write leases are exclusive. Host paths are revalidated after the old agent container is removed, closing the agent-controlled check-to-mount race; a host process that can mutate an allowlisted root remains outside the POC boundary. Treat roots as administrator-controlled and see [`docs/data-attachments.md`](./docs/data-attachments.md) for the complete boundary.
