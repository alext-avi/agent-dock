...
The worker holds delivered credentials in memory for the life of the process and never writes them to its state file. The control plane is the record. A restart therefore leaves the worker naming connectors it cannot configure: it reports them in `pendingCredentials[]`, refuses to render them, and fails a task that needs them with `missing_credential` rather than reaching the connector unauthenticated. A runtime refresh re-applies automatically; any other restart needs an apply before the agent is whole again.

Delivery is capability-gated. A worker advertises `credentialDelivery: true` and the control plane refuses to send a `credentialId` definition to one that does not, because an older worker would ignore the field and configure the connector with no header at all.
