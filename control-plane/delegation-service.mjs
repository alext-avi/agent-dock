...
export function createDelegationService(options = {}) {
  ...
  async function execute(id) {
    ...
    const promise = (async () => {
      ...
      const status = current.cancel_requested ? 'cancelled' : result.status;
      db.prepare(`
        UPDATE delegation_tasks
      ...
      `).run(
        ...
        result.output,
        status === 'cancelled' ? 'Cancellation requested by caller' : result.error,
  ...
  function submit(input, caller, policy = {}) {
    ...
    if (input.parentTaskId) {
      parent = requireRow(text(input.parentTaskId, 'parentTaskId', { required: true, max: 200 }));
      if (!caller.agentId || parent.target_agent_id !== caller.agentId) {
        throw httpError('Parent task is not assigned to the calling agent', 403);
      }
