...
The default `AUTH_MODE=trusted-local` keeps that laptop-only flow frictionless. Before exposing the control plane to another machine, enable OIDC and configure GitHub as an upstream identity through an OIDC authorization server. The login flow, role policy, deployment variables, and workload-token boundary are documented in [`docs/authentication.md`](./docs/authentication.md).
