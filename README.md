...
A credential is released only for a host on its list, checked when configuration is applied rather than when it is written, so editing a connector's destination cannot redirect it. Effort is better spent making a leaked credential cheap — narrow scope at the provider, a short host list — than on stronger encryption of a broad one.
