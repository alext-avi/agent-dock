...
| Worker wrapper | Node.js 22, built-in `http`, `child_process`, filesystem persistence; per-task supervision of the harness process group with wall-clock and idle deadlines and graceful-then-forced termination |
| Runtime limits | Provider-neutral per-agent policy stored in the registry and injected with every dispatch. Agent Dock process supervision (wall-clock timeout, idle timeout, termination grace, process-tree termination) applies to every adapter; provider-native limits (turn cap, child-command timeout and ceiling, subagent concurrency and depth, background-task policy) apply only where the adapter declares the harness exposes a control, and are reported unsupported everywhere else. Claude Code enforces all six; Codex and OpenCode enforce none. |
...
| Usage telemetry | Per-request tokens from every adapter; Codex quota windows and account activity via app-server; Claude Code quota windows only through an opt-in experimental OAuth source that is off by default; opportunistic provider-neutral fleet cache exposed through a read-only MCP summary |
