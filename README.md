...
Connector credentials can instead be managed in the UI at `/credentials`: an API key is stored once, sent only to the hosts named on it, and delivered only to the agents whose connectors reference it. Values are encrypted at rest with a per-credential data key wrapped by a key provider. **With the default provider the key comes from the control plane's own environment, so this protects a copied volume or a stray backup and nothing else** — anyone who can read this host can read the credentials. The envelope exists so a deployment with a hardware root of trust or an external key service can supply a provider instead without changing the stored format, and an operator-supplied key held only in memory is a supported shape at the cost of unattended agents being unable to apply after a restart. No key is generated automatically, because one written beside the data it protects would imply protection that does not exist.

A credential is released only for a host on its list, checked when configuration is applied rather than when it is written, so editing a connector's destination cannot redirect it — and widening the list itself requires re-supplying the value, since that is the same act as sending the credential somewhere new. Deleting one requires typing its name back, because the value exists nowhere else.

**This allowlist covers stored credentials only.** The older `MCP_SECRET_` path below has no destination allowlist: a definition carrying a legitimately provisioned connector secret can still name any `https` host. Migrating a connector to a stored credential is what closes that for it; leaving it on `sourceEnv` does not. Effort is better spent making a leaked credential cheap — narrow scope at the provider, a short host list — than on stronger encryption of a broad one.
