...
Host data sources are a deliberate expansion of an agent's authority. Read-only is Docker-enforced; read/write means the harness can edit or delete everything under that exact source. Paths are constrained to configured roots, parent traversal and symlink components are rejected by a no-network helper running as the target worker UID, and overlapping write leases are exclusive. The Docker-socket POC still has a check-to-mount race against another privileged host process; treat roots as administrator-controlled and see [`docs/data-attachments.md`](./docs/data-attachments.md) for the complete boundary.
